...
let data = [
  {
    ...
    "nim":"G2-001",
    "foto":"Assets/foto-teman/G2-001 Nina.png"
  },
  {
    "nama":"Daradina Ayu",
    "nim":"G2-002",
...
function generateRound() {
  ...
  // Simpan sample yang digunakan untuk round ini
  currentSample = data.sort(() => 0.5 - Math.random()).slice(0, 4);
  ...
  currentSample.forEach((person, index) => {
    const leftItem = document.createElement("div");
    leftItem.className = "item";
    leftItem.dataset.index = index;

    if (mode === "nama-foto") leftItem.innerText = person.nama;
    else if (mode === "nim-foto") leftItem.innerText = person.nim;
    else if (mode === "nim-nama") leftItem.innerText = person.nim;

    leftItem.onclick = () => selectItem("left", index, leftItem);
    leftColumn.appendChild(leftItem);
  });

  // Shuffle untuk kolom kanan
  const shuffled = [...currentSample].sort(() => 0.5 - Math.random());
  shuffled.forEach((person, index) => {
    const rightItem = document.createElement("div");
    rightItem.className = "item";
    
    // Cari index asli dari person ini di currentSample
    const originalIndex = currentSample.findIndex(p => p.nim === person.nim);
    ...
    if (mode === "nama-foto" || mode === "nim-foto") {
      ...
      img.onerror = function() {
        ...
        placeholder.style.cssText = 'padding: 20px; background: #f0f0f0; border: 1px dashed #ccc; text-align: center; color: #666;';
        this.parentNode.appendChild(placeholder);
      };
      rightItem.appendChild(img);
    } else if (mode === "nim-nama") {
      rightItem.innerText = person.nama;
    }

    rightItem.onclick = () => selectItem("right", originalIndex, rightItem);
...
function selectItem(side, index, element) {
  if (side === "left") {
  ...
  if (selectedLeft && selectedRight) {
    matches.push({
      ...
      right: selectedRight.dataset.index
    });
...
function checkAnswers() {
  ...
  matches.forEach(match => {
    // Bandingkan dengan currentSample, bukan sample baru
    if (match.left === match.right) {
      correct++;
    }
  });

  const salah = matches.length - correct;
  skor += correct * 2 - salah;
  document.getElementById("score").innerText = `Skor: ${skor}`;
  
  // Tampilkan feedback
  alert(`Benar: ${correct}, Salah: ${salah}, Skor bertambah: ${correct * 2 - salah}`);
